...
def get_new_constant(branch):
    ...
    for formula in branch:
        for i in range(len(formula)-1):
            if formula[i] == 'c' and i+1 < len(formula) and formula[i+1].isdigit():
...
def substitute_var_in_formula(formula, var, const):
    """
    Recursively substitute all occurrences of var with const in the entire formula,
    including within inner quantifiers
    """
    i = 0
    result = ""
    while i < len(formula):
        if i+1 < len(formula) and formula[i] in ['A', 'E']:
            # If we hit a quantifier, check if its variable is the one we're replacing
            if formula[i+1] == var:
                # Replace the quantified variable
                result += formula[i] + const + substitute_var_in_formula(formula[i+2:], var, const)
                break
            else:
                # Keep the quantifier and its variable, continue substituting in the rest
...
def sat(tableau):
    ...
    while tableau:
        ...
        if branch_state in visited:
            continue
        visited.add(branch_state)
        ...
        for formula in sorted(current_branch):
            ...
            # Handle universal quantifier
                ...
                if new_constant and constants_used < MAX_CONSTANTS:
                    # Use the substitute_var_in_formula function for proper substitution
                    new_formula = substitute_var_in_formula(subformula, var, new_constant)
                    if new_formula not in current_branch:
                        new_branch = current_branch - {formula} | {new_formula}
